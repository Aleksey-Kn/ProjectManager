    ...
    public ResponseEntity<?> setPhoto(@ModelAttribute PhotoDTO photoDTO) {
        try {
            if (photoDTO.getFile().getBytes().length > 6_291_456) {
                userService.setPhoto(jwtProvider.getLoginFromToken(),
                        compressor.compress(photoDTO.getFile()));
            } else {
                userService.setPhoto(jwtProvider.getLoginFromToken(), photoDTO.getFile().getBytes());
            }
            return new ResponseEntity<>(HttpStatus.OK);
        } catch (IOException e) {
